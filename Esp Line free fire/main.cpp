...
int g_ValidationResult = 0; // 0: Idle, 1: Validating, 2: Success, 3: Failed
            ...
            ImGui::BeginGroup();
            {
                ...
                if (activeTab == 0) {
                    ...
                    if (g_Scanning) {
                        ...
                        static char pinBuffer[64] = "";
                        static std::string statusMsg = "";
                        static ImVec4 statusColor = ImVec4(1, 1, 1, 0.4f);

                        if (g_ValidationResult == 1) {
                            statusMsg = "VALIDATING ACCESS...";
                            statusColor = ImVec4(168/255.f, 85/255.f, 247/255.f, 0.8f);
                        } else if (g_ValidationResult == 3) {
                            statusMsg = "INVALID OR EXPIRED PIN";
                            statusColor = ImVec4(1, 0.3f, 0.3f, 0.8f);
                        }

                        // Centered PIN Input
                        ImGui::SetCursorPos(ImVec2(contentWidth / 2 - 100, 90));
                        ImGui::SetNextItemWidth(200);
                        if (ImGui::InputTextWithHint("##pin", "Enter Access PIN", pinBuffer, 64)) {
                            if (g_ValidationResult == 3) g_ValidationResult = 0; // Reset error on typing
                        }

                        if (!statusMsg.empty() || g_ValidationResult != 0) {
                            ImVec2 txtSize = ImGui::CalcTextSize(statusMsg.c_str());
                            ImGui::SetCursorPos(ImVec2(contentWidth / 2 - txtSize.x / 2, 125));
                            ImGui::TextColored(statusColor, statusMsg.c_str());
                        }

                        // Centered Start Button - Ultra Clean
                        ImGui::SetCursorPos(ImVec2(contentWidth / 2 - 75, 145));
                        ...
                        bool disabled = (g_ValidationResult == 1);
                        if (disabled) ImGui::BeginDisabled();

                        if (ImGui::Button("Start Scan", ImVec2(150, 40))) {
                            if (strlen(pinBuffer) < 4) {
                                statusMsg = "INVALID PIN FORMAT";
                                statusColor = ImVec4(1, 0.3f, 0.3f, 0.8f);
                                g_ValidationResult = 3;
                            } else {
                                g_ValidationResult = 1;
                                std::string pinStr(pinBuffer);
                                std::thread([pinStr]() {
                                    g_Scanner.SetPin(pinStr);
                                    if (g_Scanner.ValidatePin()) {
                                        g_ValidationResult = 2;
                                        g_Scanning = true;
                                        g_Scanner.StartScan();
                                        g_Scanning = false;
                                        
                                        std::string reportFile = "report.html";
                                        g_Scanner.GenerateReport(reportFile);
                                    } else {
                                        g_ValidationResult = 3;
                                    }
                                }).detach();
