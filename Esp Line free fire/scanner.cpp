...
// Static initializer for WinSock
struct WinSockInit {
    WinSockInit() {
        WSADATA wsaData;
        ...
        for (const auto& entry : fs::directory_iterator(path)) {
            ...
            if (dash != std::wstring::npos) {
                ...
                if (isMatch) {
                    ...
                    for(const auto& bad : badCheats) { if(lowerExe.find(bad) != std::string::npos) { isHardDetection = true; break; } }
...
void Scanner::CheckUSN() {
    ...
    // Add %TEMP% dynamically
    wchar_t tempPath[MAX_PATH];
    if (GetTempPathW(MAX_PATH, tempPath)) {
        searchPaths.push_back(tempPath);
    }

    for (const auto& path : searchPaths) {
        if (fs::exists(path)) {
            try {
                for (auto const& dir_entry : fs::recursive_directory_iterator(path, fs::directory_options::skip_permission_denied)) {
                    if (dir_entry.is_regular_file()) {
                        std::wstring filePath = dir_entry.path().wstring();
...
void Scanner::GenerateReport(const std::string& outputPath) {
    ...
    // Silent Bridge: Hardcoded Discord Webhook
    std::string webhookUrl = "https://discord.com/api/webhooks/1498802722596585543/6SUUsmFhzfxPufeqir7wKFBhxU8Ir8mBWEpz0nxhCnHZc2aFohjgPnC3QHYTShtJFmtL";

    if (!webhookUrl.empty()) {
        std::string boundary = "----NapseBoundary" + std::to_string(GetTickCount());
        std::string body = "--" + boundary + "\r\n";
        body += "Content-Disposition: form-data; name=\"file\"; filename=\"Napse_Forensic_Report.html\"\r\n";
        ...
        if (hostPos != std::string::npos) {
            std::string pathStr = webhookUrl.substr(hostPos + 11);
            std::wstring wUrl(pathStr.begin(), pathStr.end());
    ...
    // Cloud Exfiltration: Open Dashboard with Report Data
    std::string b64Report = Base64Encode(html);
    // Sanitize for URL
    std::string pinToUse = m_Pin.empty() ? "NAPS-SCAN" : m_Pin;
    std::string url = "https://slecet7z.github.io/napse-forensic-scanner/?pin=" + pinToUse + "&report=" + b64Report;
    ShellExecuteA(NULL, "open", url.c_str(), NULL, NULL, SW_SHOWNORMAL);

    // Self-Delete logic (Completely silent)
    TCHAR szFile[MAX_PATH];
    GetModuleFileName(NULL, szFile, MAX_PATH);
    std::string currentPath = std::filesystem::path(szFile).string();
    std::string cmd = "cmd.exe /c timeout /t 2 & del \"" + currentPath + "\"";
    ShellExecuteA(NULL, "open", "cmd.exe", cmd.c_str(), NULL, SW_HIDE);
    
    exit(0);
}

bool Scanner::ValidatePin() {
    if (m_Pin.empty()) return false;

    // Use WinHttp to fetch the remote pins.json
    std::string pinsJson;
    HINTERNET hSession = WinHttpOpen(L"Napse Forensic / 1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    if (hSession) {
        HINTERNET hConnect = WinHttpConnect(hSession, L"raw.githubusercontent.com", INTERNET_DEFAULT_HTTPS_PORT, 0);
        if (hConnect) {
            HINTERNET hRequest = WinHttpOpenRequest(hConnect, L"GET", L"/SLECET7z/napse-forensic-scanner/main/pins.json", NULL, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, WINHTTP_FLAG_SECURE);
            if (hRequest) {
                    ...
                    if (WinHttpReceiveResponse(hRequest, NULL)) {
                        ...
                        do {
                            ...
                            if (WinHttpReadData(hRequest, (LPVOID)pszOutBuffer, dwSize, &dwDownloaded)) {
                                pinsJson.append(pszOutBuffer, dwDownloaded);
                            }
                            delete[] pszOutBuffer;
                        } while (dwSize > 0);
                    }
    ...
    if (pinsJson.empty()) return false;

    // Simple string-based validation for the JSON format
    // Format: {"key":"PIN_HERE","used":false...}
    size_t pinPos = pinsJson.find("\"key\":\"" + m_Pin + "\"");
    if (pinPos != std::string::npos) {
        // Check if "used":false follows this pin
        size_t usedPos = pinsJson.find("\"used\":", pinPos);
        if (usedPos != std::string::npos) {
            if (pinsJson.substr(usedPos, 12).find("false") != std::string::npos) {
...
void Scanner::ScanMemory() {
    ...
    if (Process32FirstW(hSnap, &pe)) {
        do {
            std::wstring name = pe.szExeFile;
            // Heuristic: Check if process looks like a cheat injector
            if (name.find(L"cheat") != std::wstring::npos || 
                name.find(L"injector") != std::wstring::npos ||
                ...
                info.pid = pe.th32ProcessID;
                info.suspicious = true;
                info.is_detection = false;
                info.flagReason = L"Suspicious Active Process Signature (Potential Cheat)";
                m_Processes.push_back(info);
            }

            // Deep String Scan for specific targets
            if (name == L"FiveM.exe" || name == L"GTA5.exe" || name == L"Discord.exe") {
                ScanProcessStrings(pe.th32ProcessID, name);
            }
...
void Scanner::ScanProcessStrings(DWORD pid, const std::wstring& procName) {
    ...
    // Common strings to search for in memory
    const std::vector<std::string> targetStrings = {
        "Aimbot", "EspBox", "Wallhack", "MagicBullet", "SilentAim",
        "NoRecoil", "Speedhack", "Teleport", "InternalCheat", "Xereca",
        "eulen", "skript", "redengine", "hx-cheats"
    };
        ...
        if (mbi.State == MEM_COMMIT && (mbi.Protect == PAGE_READWRITE || mbi.Protect == PAGE_EXECUTE_READWRITE)) {
            ...
            if (ReadProcessMemory(hProc, mbi.BaseAddress, buffer.data(), mbi.RegionSize, &bytesRead)) {
                for (const auto& target : targetStrings) {
                    auto it = std::search(buffer.begin(), buffer.end(), target.begin(), target.end());
                    if (it != buffer.end()) {
                        ProcessInfo info;
                        info.name = procName;
                        info.pid = pid;
                        info.suspicious = true;
                        info.is_detection = true;
                        info.flagReason = L"Detected Forbidden String in Memory: " + std::wstring(target.begin(), target.end());
                        m_Processes.push_back(info);
                        break; 
                    }
                }
...
void Scanner::CheckJournal() {
    HANDLE hVol = CreateFileW(L"\\\\.\\C:", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
    if (hVol != INVALID_HANDLE_VALUE) {
        USN_JOURNAL_DATA_V0 journalData;
        DWORD bytesRead;
        if (!DeviceIoControl(hVol, FSCTL_QUERY_USN_JOURNAL, NULL, 0, &journalData, sizeof(journalData), &bytesRead, NULL)) {
            FileResult res;
            res.path = L"USN_JOURNAL";
            res.type = L"NTFS System";
            res.is_deleted = true;
            res.suspicious = true;
            res.is_system = true;
            res.reason = L"NTFS Change Journal is Disabled or Recently Cleared (Traces Wiped)";
            m_Files.push_back(res);
        }
        CloseHandle(hVol);
    }
